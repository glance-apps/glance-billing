...
import { ipcMain, inAppPurchase, net, app } from 'electron';
import fs from 'node:fs';
import path from 'node:path';
import crypto from 'node:crypto';
const RC_BASE = 'https://api.revenuecat.com/v1';
let cachedPrices = { yearly: null, lifetime: null, yearlyTrialDays: null };
...
function lifetimeLatchPath(o) {
    return path.join(app.getPath('userData'), o.latchFileName);
}
function isLifetimeLatched(o) {
    try {
        return fs.existsSync(lifetimeLatchPath(o));
    }
    catch {
        return false;
    }
}
function latchLifetime(o, source) {
    try {
        if (isLifetimeLatched(o))
            return;
...
function fireBillingEvent(payload) {
...
function settleRestore(active, productId) {
    if (!restorePending)
        return;
...
// ── RevenueCat REST API ───────────────────────────────────────────────────────
async function rcFetch(o, method, endpoint, body) {
    const opts = {
        method,
        headers: {
...
export function parseEntitlement(subscriber, entitlementId) {
    if (subscriber == null)
        return null; // couldn't read a subscriber at all
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const ent = subscriber?.entitlements?.[entitlementId];
    if (!ent)
        return { active: false, productId: null }; // subscriber exists, no entitlement
...
async function fetchEntitlementStatus(o) {
    const appUserId = getStableAnonymousId();
    const latched = isLifetimeLatched(o);
    let receiptPath = null;
    try {
        receiptPath = inAppPurchase.getReceiptURL();
    }
    catch {
        receiptPath = null;
    }
    ...
    if (hasReceipt) {
        try {
            ...
            console.info(`${o.logTag} POST /receipts entitlements =`, JSON.stringify(sub?.entitlements ?? null)?.slice(0, 400));
            if (sub && typeof sub.entitlements === 'object') {
                const parsed = parseEntitlement(sub, o.entitlementId);
                if (parsed.active && parsed.productId === o.products.lifetime)
                    latchLifetime(o, 'rc-receipt-validation');
                if (!parsed.active && latched)
                    return { active: true, productId: o.products.lifetime };
            ...
            console.warn(`${o.logTag} receipt validation failed (indeterminate):`, err instanceof Error ? err.message : err);
        }
    }
    ...
    try {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const parsed = parseEntitlement(data?.subscriber, o.entitlementId);
        console.info(`${o.logTag} GET /subscribers entitlement active =`, parsed?.active);
        if (parsed?.active) {
            if (parsed.productId === o.products.lifetime)
                latchLifetime(o, 'rc-subscriber-lookup');
            return parsed;
        }
    }
    catch (err) {
...
// hardcoded number.
function trialDaysFromProduct(product) {
    const intro = product.introductoryPrice;
    if (!intro || intro.paymentMode !== 'freeTrial')
        return null;
    const period = intro.subscriptionPeriod;
    if (!period)
        return null;
    const unitDays = period.unit === 'day' ? 1 :
        period.unit === 'week' ? 7 :
...
// ── Registration ─────────────────────────────────────────────────────────────
export function registerElectronBilling(window, options) {
    const o = {
    ...
    if (process.platform === 'darwin' && inAppPurchase.canMakePayments()) {
        inAppPurchase.getProducts([o.products.yearly, o.products.lifetime]).then((products) => {
            const prices = { yearly: null, lifetime: null, yearlyTrialDays: null };
            for (const p of products) {
                if (p.productIdentifier === o.products.yearly) {
                    prices.yearly = p.formattedPrice;
                    prices.yearlyTrialDays = trialDaysFromProduct(p);
                }
                if (p.productIdentifier === o.products.lifetime)
                    prices.lifetime = p.formattedPrice;
            }
            cachedPrices = prices;
        ...
        inAppPurchase.on('transactions-updated', (async (_event, transactions) => {
            for (const t of transactions) {
                if (t.transactionState === 'purchased' || t.transactionState === 'restored') {
                    ...
                    // bookkeeping.
                    if (t.payment.productIdentifier === o.products.lifetime)
                        latchLifetime(o, 'storekit-transaction');
    ...
    ipcMain.handle('subscription:purchase', async (_event, productId) => {
        if (process.platform !== 'darwin' || !inAppPurchase.canMakePayments()) {
