import { DEFAULT_STORAGE_KEYS, DEFAULT_TIMINGS, } from './config.js';
import { SafeStorage } from './storage.js';
import { deriveReviewerCode, sha256Hex } from './reviewer.js';
const INACTIVE = { active: false, productId: null };
/**
...
export class BillingEngine {
    ...
    hintGrantsProvisionalUnlock(hint) {
        if (this.offlineGraceDays === undefined)
            return true;
        ...
        if (typeof hint.verifiedAt !== 'number')
            return true;
    ...
    applyStatus(next) {
        ...
        if (next.active) {
            ...
            const hint = { productId: next.productId ?? null, verifiedAt: this.now() };
        ...
        if (this.downgradeTimer)
            return; // a downgrade is already pending confirmation
    ...
    consumeTestPurchase() {
        if (!this.adapter?.consumeTestPurchase)
            return;
        this.setState({ billingEvent: null });
    ...
    // ── Reviewer bypass ────────────────────────────────────────────────────────
    async restoreReviewerUnlock() {
        if (!this.reviewerSecret)
            return;
    ...
    async setReviewerUnlocked(input) {
        if (!this.reviewerSecret)
            return false;
        try {
    ...
    buildSnapshot() {
        ...
        const entitlementSource = !gated ? 'channel'
